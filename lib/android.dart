...
overwriteExistingIcons(AndroidIconTemplate e, image, filename) {
  ...
  if (image.width >= e.size) {
    newFile = copyResize(image, width: e.size, height: -1, interpolation: AVERAGE);
  } else {
    newFile = copyResize(image, width: e.size, height: -1, interpolation: LINEAR);
  }
...
saveNewImages(AndroidIconTemplate template, image, String iconFilePath) {
  ...
  if (image.width >= template.size) {
    newFile = copyResize(image, width: template.size, height: template.size, interpolation: AVERAGE);
  }
  else {
    newFile = copyResize(image, width: template.size, height: template.size, interpolation: LINEAR);
  }
